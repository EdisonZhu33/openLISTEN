...
class FiLMFusion(nn.Module):
    ...
    用于消融实验：对比 GCA 和 FiLM 的融合效果
    
    工作原理（与 GCA 对齐）:
    - GCA: audio 作为 Query, text 作为 K/V → 输出 [B, S, D]
    - FiLM: 用 text 的全局表示调制 audio → 输出 [B, S, D]
    
    这样 FiLM 的输出形状与 GCA 完全一致，无需修改下游代码。
    """
    def __init__(self, text_dim: int, audio_dim: int):
        ...
        # 从 text 全局表示生成调制参数（用于调制 audio）
        self.gamma_proj = nn.Linear(text_dim, audio_dim)
        self.beta_proj = nn.Linear(text_dim, audio_dim)
        
        # 层归一化
        self.norm = nn.LayerNorm(audio_dim)
        
        # 初始化：gamma 初始化为接近1，beta 初始化为接近0
    ...
    def forward(self, audio_emb: torch.Tensor, text_emb: torch.Tensor) -> torch.Tensor:
        """
        Args:
            audio_emb: 音频嵌入 [batch_size, audio_seq_len, audio_dim] - 被调制目标
            text_emb: 文本嵌入 [batch_size, text_seq_len, text_dim] - 调制条件
        Returns:
            调制后的音频嵌入 [batch_size, audio_seq_len, audio_dim]
            
        注意：输出形状与 audio_emb 相同，与 GCA 输出形状一致！
        """
        # 1. 文本全局池化 (mean pooling) 作为条件
        text_global = text_emb.mean(dim=1)  # [B, text_dim]
        
        # 2. 生成调制参数
        gamma = self.gamma_proj(text_global)  # [B, audio_dim]
        beta = self.beta_proj(text_global)    # [B, audio_dim]
        
        # 3. 扩展到序列维度以进行广播
        gamma = gamma.unsqueeze(1)  # [B, 1, audio_dim]
        beta = beta.unsqueeze(1)    # [B, 1, audio_dim]
        
        # 4. FiLM 调制: output = gamma * audio + beta
        # 使用 (1 + gamma) 使初始状态更稳定
        modulated = (1 + gamma) * audio_emb + beta
        
        # 5. 层归一化
        output = self.norm(modulated)
...
class OmniSpeechMetaModel:
    ...
    def __init__(self, config):
        ...
        # FiLM 融合模块
        self.film_fusion = FiLMFusion(config.hidden_size, config.hidden_size)
...
class OmniSpeechMetaForCausalLM(ABC):
    ...
    def get_film_fusion(self):
        """获取 FiLM 融合模块（用于消融实验）"""
        return self.get_model().film_fusion
    ...
    def fuse_modalities(self, audio_emb, text_emb):
        """
        多模态特征融合 - 【消融实验】使用 FiLM 融合
        
        FiLM (Feature-wise Linear Modulation) 原理:
        - 从 audio 特征生成 gamma 和 beta 调制参数
        - 对 text 特征进行仿射变换: output = gamma * text + beta
        
        注意：参数顺序与调用处一致！
        :param audio_emb: [B, S, D] 音频嵌入（调制条件，用于生成 gamma/beta）
        :param text_emb: [B, T, D] 文本嵌入（被调制的目标）
        :return: [B, T, D] FiLM 调制后的文本嵌入
        """
        # ============ FiLM 融合 ============
        # FiLMFusion.forward(audio_emb, text_emb) - 用 text 条件调制 audio
        # 输出形状 [B, S, D] 与 GCA 一致
        film_module = self.get_film_fusion()
        fused_out = film_module(audio_emb, text_emb)
        
        # ============ 原始 GCA 融合（已注释）============
        # 原 GCA 中：audio 作为 Query, text 作为 K/V
        # cross_attns = self.get_cross_attn()
        # gates_projs = self.get_gates()
        # attn_text = cross_attns(audio_emb, text_emb)  # audio作为Q, text作为K/V
        ...
        # fused_out = gate * attn_text + (1 - gate) * audio_context.expand_as(attn_text)
    ...
    def plot_embedding_abs_means(self,text_emb_list, audio_emb, final_embs):
        """
        Compute and plot the average absolute values for each row (i.e. index) of three embeddings:
          - Text Embedding: computed from torch.cat(text_emb_list)
          - Audio Embedding: audio_emb
          - Final Embedding: final_embs

        Each embedding produces a line (with different colors) representing the average absolute value
        over the embedding dimension at each index. The plot includes a legend at the upper right corner.
        """
        # Compute average absolute value along the embedding dimension for each index.
    ...
    def plot_embedding_clusters5(self, text_emb_list, audio_emb, final_embs):
        ...
        plt.tight_layout()
        # 新增保存矢量图逻辑 [[5]][[7]]
        output_path = "/home/chengz/Code/sft_chatglm/text_json.svg"  # 支持svg/pdf/eps等矢量格式
        # 新增多格式保存逻辑 [[4]][[5]]
        base_filename = "/home/chengz/Code/sft_chatglm/Ablation Studies3"  # 基础文件名

        formats = ['svg', 'pdf', 'png']  # 需要保存的格式列表

        for fmt in formats:
            output_path = f"{base_filename}.{fmt}"

            if fmt == 'png':  # 特殊处理光栅图格式 [[4]]
    ...
    def prepare_inputs_labels_for_speech_and_text(
        ...
        if labels !=None:
            ...
            # 推理时使用完整的 text_emb_list（不切分）
            text_emb_list_p = torch.cat(text_emb_list_1)  # [全部 text tokens, D]
        ...
        # text_emb = torch.cat(text_emb_list)
        ...
        # Update new_labels to match the new final_embs
        # 注意：现在 FiLM 输出形状与 GCA 一致 [B, S, D]，下面的重建逻辑不再需要
        # 如果仍有形状不匹配问题，可取消下面的注释
        # if _labels is not None:
        #     text_len = text_emb.shape[0]
        #     fused_len = final_emb.shape[1]  # FiLM/GCA 输出长度
        #     original_labels = torch.cat(new_labels)
        #     if labels != None:
        #         labels_len = only_lables_embs.shape[1]
        #         new_labels_tensor = torch.cat([
        #             torch.full((text_len,), IGNORE_INDEX, device=original_labels.device, dtype=original_labels.dtype),
        #             torch.full((fused_len,), IGNORE_INDEX, device=original_labels.device, dtype=original_labels.dtype),
        #             original_labels[-labels_len:]
        #         ])
        #     else:
        #         new_labels_tensor = torch.cat([
        #             torch.full((text_len,), IGNORE_INDEX, device=original_labels.device, dtype=original_labels.dtype),
        #             torch.full((fused_len,), IGNORE_INDEX, device=original_labels.device, dtype=original_labels.dtype)
        #         ])
        #     new_labels_tensor = new_labels_tensor[:final_embs.shape[0]]
        #     new_labels = [new_labels_tensor]
